...
class GateController:
    ...
    def stop_gate(self):
        self.motor_direction.value(0)  # Just don't supply power to the relay
        self.motor_enable.value(0)  # Stop motor


class GarageDoorSystem:
    def __init__(self):
        self.left_gate = GateController(K1_MOTOR_1, K2_MOTOR_1, LEFT_OPEN, LEFT_CLOSE)
        self.right_gate = GateController(
        ...
        self.timer = Timer(-1)
        self.gate_open_time = 10  # Default time to keep the gate open
        self.gate_closing = False
        self.check_gate_status_on_boot()

    def check_gate_status_on_boot(self):
        if not self.left_gate.is_closed() or not self.right_gate.is_closed():
            self.close_gates()

    def start_system(self, pin):
        if not self.gate_closing:
            self.open_gates()

    def open_gates(self):
        self.left_gate.open_gate()
        utime.sleep(self.left_gate.delay)
    ...
    def keep_gate_open(self):
        ...
        while utime.time() - start_time < self.gate_open_time:
    ...
    def close_gates(self):
        ...
        utime.sleep(self.right_gate.delay)
    ...
    def monitor_gates(self, gate1, gate2):
        while not gate1.is_open() and not gate1.is_closed():
            utime.sleep(0.1)
        gate1.stop_gate()
        while not gate2.is_open() and not gate2.is_closed():
            utime.sleep(0.1)
        gate2.stop_gate()


system = GarageDoorSystem()
while True:
    utime.sleep(1)  # Keep the script running
